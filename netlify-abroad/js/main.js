...
/* ── LANGUAGE ─────────────────────────────── */
const LANG_STORAGE_KEY = 'td_lang';
const LANG_MANUAL_KEY = 'td_lang_manual';
const LANG_GEO_SESSION_KEY = 'td_geo_lang';
const GEO_COUNTRY_ENDPOINTS = [
...
function validLang(lang) {
  return T[lang] ? lang : null;
}

function getManualLang() {
  if (storageGet(localStorage, LANG_MANUAL_KEY) !== 'true') return null;
  return validLang(storageGet(localStorage, LANG_STORAGE_KEY));
}
...
function applyLang(lang, options = {}) {
  ...
  currentLang = lang;
  if (options.persist) {
    storageSet(localStorage, LANG_STORAGE_KEY, lang);
    storageSet(localStorage, LANG_MANUAL_KEY, 'true');
  }
...
async function fetchCountryCode() {
  for (const endpoint of GEO_COUNTRY_ENDPOINTS) {
    try {
      const response = await fetchWithTimeout(endpoint);
      if (!response.ok) continue;
      if (endpoint.includes('geojs.io')) {
        const data = await response.json();
        if (data && data.country) return String(data.country).toUpperCase();
      } else {
        const text = await response.text();
        if (text) return text.trim().slice(0, 2).toUpperCase();
      }
    } catch (error) {
      // Try the next provider, then fall back to English.
    }
  }
  return '';
}
...
async function detectGeoLang() {
  const cached = validLang(storageGet(sessionStorage, LANG_GEO_SESSION_KEY));
  if (cached) return cached;
  const country = await fetchCountryCode();
  const lang = country === 'RU' ? 'ru' : 'en';
  storageSet(sessionStorage, LANG_GEO_SESSION_KEY, lang);
  return lang;
}

async function initGeoLanguage() {
  applyLang('en');
}
...
/* ── CALCULATOR ───────────────────────────── */
const CALC_SERVICES = [
  { icon:'WEB', nKey:'c1n', dKey:'c1d', min:1200,  max:4500,   id:'c1' },
  { icon:'SHOP', nKey:'c2n', dKey:'c2d', min:4500,  max:12000,  id:'c2' },
  { icon:'APP', nKey:'c3n', dKey:'c3d', min:9000,  max:25000,  id:'c3' },
  { icon:'AI', nKey:'c4n', dKey:'c4d', min:2500,   max:18000,  id:'c4', tiersKey:'c4tiers' },
  { icon:'3D', nKey:'c5n', dKey:'c5d', min:3500,   max:12000,  id:'c5' },
  { icon:'SYS', nKey:'c6n', dKey:'c6d', min:20000, max:90000,  id:'c6' },
];
...
const EJ_TO_EMAILS = ['n.nate1544@gmail.com'];
